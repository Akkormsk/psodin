...
class Config:
    ...
    SQLALCHEMY_DATABASE_URI = os.getenv('DATABASE_URL', f"sqlite:///{os.path.join(basedir, 'print_shop.db')}")
    ...
    SESSION_FILE_DIR = os.getenv('SESSION_FILE_DIR', os.path.join(basedir, 'flask_session'))
    # Путь к лог-файлу; по умолчанию /tmp/app.log, чтобы избежать ошибок записи
    LOG_FILE = os.getenv('LOG_FILE', os.path.join(basedir, 'app.log'))
